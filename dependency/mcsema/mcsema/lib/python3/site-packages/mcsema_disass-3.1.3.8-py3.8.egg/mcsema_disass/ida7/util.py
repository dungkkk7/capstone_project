...
import collections
import idaapi
# Shim for IDA 9+ compatibility
if not hasattr(idaapi, "get_inf_structure"):
    class InfStructureWrapper(object):
        @property
        def procName(self):
            if hasattr(idaapi, "inf_get_procname"):
                return idaapi.inf_get_procname()
            return ""

        @property
        def filetype(self):
            if hasattr(idaapi, "inf_get_filetype"):
        ...
        def is_32bit(self):
            if hasattr(idaapi, "inf_is_32bit_exactly"):
                return idaapi.inf_is_32bit_exactly()
            return not self.is_64bit()

    def get_inf_structure_shim():
        return InfStructureWrapper()

    idaapi.get_inf_structure = get_inf_structure_shim
