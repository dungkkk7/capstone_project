...
from binary_lifting.lifting2 import lift_binary
...
def main(argv=None):
    ...
    # Kiểm tra tham số
    if len(argv) < 1:
        print(f"{Color.YELLOW}Usage: python main3.py <obfuscated_bin_list.csv>{Color.END}")
        return 1
    ...
    success_count = 0
    lift_start_time = time.perf_counter()
    for path in binary_paths:
        ...
        # Gọi cleanup-ir lifter, truyền output_bc chỉ định thư mục pipeline con
    ...
    print(f"{Color.BLUE}[*] Thời gian lift toàn bộ các tệp được yêu cầu: {format_elapsed_time(time.perf_counter() - lift_start_time)}{Color.END}")
