// Deterministic program-level Ghidra decompiler export for B0 experiments.
    ...
    public void run() throws Exception {
        ...
        while (dataIterator.hasNext() && !monitor.isCancelled()) {
            ...
            MemoryBlock dataBlock =
                currentProgram.getMemory().getBlock(data.getAddress());
            if (dataBlock == null) {
                continue;
            }
            String blockName = dataBlock.getName();
            if (!(blockName.equals(".rodata") ||
                  blockName.equals(".data") ||
                  blockName.equals(".bss") ||
                  blockName.equals(".got") ||
                  blockName.equals(".got.plt"))) {
                continue;
            }
            Symbol symbol = data.getPrimarySymbol();
            String label =
                symbol == null ? "<unnamed>" : symbol.getName();
            String value = data.getDefaultValueRepresentation();
            out.println(
                "/* GLOBAL " + data.getAddress() + " " + label + " = " + value + " */"
            );
        }
        out.println();
