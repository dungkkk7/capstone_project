...
DEFAULT_REVNG_ARTIFACT = "cleanup-ir"
CLEAN_IR_ARTIFACT_CHAIN = [
    # "lift",
    # "isolate",
    # "enforce-abi",
    # "simplify-switch",
...
def decode_revng_artifact(artifact_output, output_bc):
    ...
    if decoded.startswith(ZSTD_MAGIC):
        zstd = shutil.which("zstd")
        if not zstd:
            print(f"{Color.RED}[FAIL] Artifact cua revng dang nen zstd nhung khong tim thay lenh zstd.{Color.END}")
            return False
        ...
        with tempfile.NamedTemporaryFile(suffix=".zst", delete=False) as tmp:
            ...
            tmp_zst = tmp.name
        try:
            ok = run_command([zstd, "-d", "-f", tmp_zst, "-o", output_bc], "Giai nen artifact cleanup-ir (.zstd)")
            return bool(ok)
        finally:
            try:
                os.unlink(tmp_zst)
...
def run_revng_artifact(root, project_dir, artifact, capture_output=False):
    artifact_cmd = revng2_cmd(root) + ["-C", project_dir, "project", "artifact", artifact]
    return run_command(
        artifact_cmd,
        f"Chay artifact revng '{artifact}'",
        capture_output=capture_output,
    )


def normalize_artifact_plan(artifact):
    if artifact in ("auto", "best", "cleanest", DEFAULT_REVNG_ARTIFACT):
        return CLEAN_IR_ARTIFACT_CHAIN, DEFAULT_REVNG_ARTIFACT
    return [artifact], artifact


def lift_binary(binary_path, output=None, keep_project=False, artifact="auto"):
    ...
    artifact_plan, final_artifact = normalize_artifact_plan(artifact)
    print(f"{Color.BLUE}[*] Artifact plan: {' -> '.join(artifact_plan)}{Color.END}")
    print(f"{Color.BLUE}[*] Artifact xuat ra IR cuoi: {final_artifact}{Color.END}")

    artifact_output = b""
    for index, current_artifact in enumerate(artifact_plan, start=1):
        print(f"{Color.BLUE}{Color.BOLD}    [Artifact {index}/{len(artifact_plan)}] {current_artifact}{Color.END}")
        result = run_revng_artifact(
            root,
            project_dir,
            current_artifact,
            capture_output=True,
        )
        ok, current_output = result
        if current_artifact == final_artifact:
            artifact_output = current_output
        if not ok:
            return False

    if not decode_revng_artifact(artifact_output, output_bc):
        return False
    print(f"{Color.GREEN}      [OK] Da ghi LLVM bitcode sach nhat tu artifact '{final_artifact}': {output_bc}{Color.END}")
...
def main():
    parser = argparse.ArgumentParser(description="revng best-effort clean LLVM IR lifter")
    ...
    parser.add_argument(
        "--artifact",
        default="auto",
        help="Artifact revng can xuat. Dung auto/best de chay chuoi clean IR day du (mac dinh: auto).",
    )
