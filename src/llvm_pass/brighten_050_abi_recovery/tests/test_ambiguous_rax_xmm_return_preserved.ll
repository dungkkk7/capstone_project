; McSema callers may reload both RAX and XMM0 after every call.  When both
; channels have live-out evidence but XMM0 is independent of RAX, ABI recovery
; must not guess that the XMM scratch value is the source-level return.

target datalayout = "e-m:e-p270:32:32-p271:32:32-p272:64:64-i64:64-i128:128-f80:128-n8:16:32:64-S128"
target triple = "x86_64-pc-linux-gnu"

@__mcsema_reg_state = internal global [3376 x i8] zeroinitializer

define ptr @sub_ambiguous(ptr %state, i64 %pc, ptr %memory) {
entry:
  %typed.xmm0 = alloca i64, align 8, !brighten.state.abi_type !0
  store i64 4607182418800017408, ptr %typed.xmm0, align 8
  %fp.bits = load i64, ptr %typed.xmm0, align 8
  %xmm0 = getelementptr i8, ptr %state, i64 16
  store i64 %fp.bits, ptr %xmm0, align 8
  %rax = getelementptr i8, ptr %state, i64 2216
  store i64 7, ptr %rax, align 8
  ret ptr %memory
}

define i64 @observe_both() {
entry:
  %memory = call ptr @sub_ambiguous(
      ptr @__mcsema_reg_state, i64 0, ptr null)
  %rax.ptr = getelementptr i8, ptr @__mcsema_reg_state, i64 2216
  %rax = load i64, ptr %rax.ptr, align 8
  %xmm.ptr = getelementptr i8, ptr @__mcsema_reg_state, i64 16
  %xmm = load double, ptr %xmm.ptr, align 8
  call void @consume_double(double %xmm)
  ret i64 %rax
}

declare void @consume_double(double)

; CHECK: define internal i64 @sub_ambiguous.native
; CHECK-NOT: define internal double @sub_ambiguous.native

!0 = !{!"double"}
