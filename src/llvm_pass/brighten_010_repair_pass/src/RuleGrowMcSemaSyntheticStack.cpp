...
static bool MentionsOldStack(Value *V, Value *OldStack) {
  if (V == OldStack) return true;
  if (auto *I = dyn_cast<Instruction>(V)) {
    for (Value *Op : I->operands()) {
      if (MentionsOldStack(Op, OldStack)) return true;
    }
  } else if (auto *C = dyn_cast<Constant>(V)) {
    if (C == OldStack) return true;
  ...
  for (Function &F : M) {
    for (BasicBlock &BB : F) {
      for (Instruction &I : BB) {
        ...
        if (MentionsOldStack(Val, OldStack)) {
