...
bool BrightenStateSSAPass::PromoteStateToSSA(Module &M) {
  ...
  for (Function &F : M) {
    ...
    // DenseMap iteration is intentionally unordered.  Keep the generated
    // architectural cells stable so metadata/debug output and downstream
    // lifecycle passes see a deterministic state layout.
    llvm::sort(Bases);
    ...
    for (auto &Entry : Cells) {
      ...
      AllocaInst *Alloca = EntryBuilder.CreateAlloca(
          CellTy, nullptr, "state_" + std::to_string(Cell.BaseOffset));
      Alloca->setMetadata(
    ...
    auto flushCells = [&](IRBuilder<> &B) {
      for (auto &Entry : Cells) {
        uint64_t Base = Entry.first;
        IntegerType *CellTy = CellTypes[Base];
        Value *LoadedValue = B.CreateLoad(CellTy, CellAllocas[Base],
                                          "state.cell.flush");
        Value *Ptr = statePointer(B, StatePtr, Base, "state.cell.flush.ptr");
        createUnalignedStore(B, LoadedValue, Ptr);
      }
    };
    auto reloadCells = [&](IRBuilder<> &B) {
      for (auto &Entry : Cells) {
        ...
        Value *LoadedValue = createUnalignedLoad(
            B, CellTy, Ptr, "state.cell.reload");
        B.CreateStore(LoadedValue, CellAllocas[Base]);
      }
    };
